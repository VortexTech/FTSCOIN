                                                 ...
                                                 m_addressProvider(new AddressProvider(this))
{
  ...
  const QUrl url = QUrl::fromUserInput("https://explorer.FTSCoin.network/services/charts/price.png?vsCurrency=usd&days=7&priceDecimals=2&xPoints=12&width=511&height=191&dateFormat=MM-DD");
...
/* Update the total portfolio in FTS and Fiat on the top left hand corner */
void OverviewFrame::updatePortfolio()
{
  ...
  m_ui->m_totalPortfolioLabelUSD->setText(tr("TOTAL") + " " + CurrencyAdapter::instance().formatAmount(OverviewFrame::totalBalance) + " FTS | " + CurrencyAdapter::instance().formatCurrencyAmount(total / 10000) + " " + Settings::instance().getCurrentCurrency());
}
...
void OverviewFrame::bankingClicked()
{
  ...
  if (walletSynced == true)
  {
    m_ui->m_myFTSCoinWalletTitle->setText("BANKING");
...
{
  m_ui->darkness->hide();
  m_ui->m_myFTSCoinWalletTitle->setText("TRANSACTIONS");
...
{
  m_ui->darkness->hide();
  m_ui->m_myFTSCoinWalletTitle->setText("CONCEAL.NETWORK");
...
{
  m_ui->darkness->hide();
  m_ui->m_myFTSCoinWalletTitle->setText("ABOUT");
  m_ui->aboutBox->raise();
  m_ui->m_newTransferButton->show();
  m_ui->m_newMessageButton->show();
}
...
{
  m_ui->darkness->hide();
  m_ui->m_myFTSCoinWalletTitle->setText("WALLET SETTINGS");
...
{
  m_ui->darkness->hide();
  m_ui->m_myFTSCoinWalletTitle->setText("INBOX");
...
void OverviewFrame::newTransferClicked()
{
  ...
  if (walletSynced == true)
  {
    m_ui->m_myFTSCoinWalletTitle->setText("SEND FUNDS");
    m_ui->sendBox->raise();
...
void OverviewFrame::newMessageClicked()
{
  ...
  if (walletSynced == true)
  {
    m_ui->m_myFTSCoinWalletTitle->setText("NEW MESSAGE");
...
void OverviewFrame::setAddress(const QString &_address)
{
  ...
  else
  {
    ...
    m_ui->m_myFTSCoinWalletTitle->setText("SEND MESSAGE");
    m_ui->newMessageBox->raise();
  }
}
...
void OverviewFrame::onAddressFound(const QString &_address)
{
  ...
  if ((!_address.isEmpty()) && (connection != "embedded"))
  {
    ...
    m_ui->m_sendFee->setText("Fee: 0.011000 FTS");
    m_ui->m_messageFee->setText("Fee: 0.011000 FTS");
    m_ui->m_depositFeeLabel->setText("0.011000 FTS");
  }
}
...
void OverviewFrame::sendFundsClicked()
{
  ...
  try
  {
    ...
    {
      /* Parse the record and set address to the actual FTS address */
      std::vector<std::string> records;
      if (!Common::fetch_dns_txt(address.toStdString(), records))
      {
        QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Failed to lookup FTSCoin ID"), QtCriticalMsg));
      }
  ...
  {
    QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Could not check FTSCoin ID"), QtCriticalMsg));
    return;
  }
...
{
  m_ui->m_myFTSCoinWalletTitle->setText("ADDRESS BOOK");
...
{
  QDesktopServices::openUrl(QUrl("http://discord.FTSCoin.network/", QUrl::TolerantMode));
}

void OverviewFrame::twitterClicked()
{
  QDesktopServices::openUrl(QUrl("https://twitter.com/FTSCoinNetwork", QUrl::TolerantMode));
}
...
{
  QDesktopServices::openUrl(QUrl("https://github.com/FTSCoinNetwork", QUrl::TolerantMode));
}

void OverviewFrame::redditClicked()
{
  QDesktopServices::openUrl(QUrl("https://www.reddit.com/r/FTSCoinNetwork/", QUrl::TolerantMode));
}

void OverviewFrame::mediumClicked()
{
  QDesktopServices::openUrl(QUrl("https://medium.com/@FTSCoinNetwork", QUrl::TolerantMode));
}

void OverviewFrame::hotbitClicked()
{
  QDesktopServices::openUrl(QUrl("https://www.hotbit.io/exchange?symbol=FTS_BTC", QUrl::TolerantMode));
}

void OverviewFrame::stexClicked()
{
  QDesktopServices::openUrl(QUrl("https://app.stex.com/en/basic-trade/pair/BTC/FTS", QUrl::TolerantMode));
}

void OverviewFrame::tradeogreClicked()
{
  QDesktopServices::openUrl(QUrl("https://tradeogre.com/exchange/BTC-FTS", QUrl::TolerantMode));
}

void OverviewFrame::qtradeClicked()
{
  QDesktopServices::openUrl(QUrl("https://qtrade.io/market/FTS_BTC", QUrl::TolerantMode));
}

void OverviewFrame::helpClicked()
{
  QDesktopServices::openUrl(QUrl("https://FTSCoin.network/wiki/doku.php?id=start", QUrl::TolerantMode));
}
