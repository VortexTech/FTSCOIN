...
void Updater::checkForUpdate()
{
  ...
  const QUrl url = QUrl::fromUserInput("http://walletapi.FTSCoin.network/version.txt");
    ...
    else
    {
        ...
        Version remoteVersion = result.toStdString();
         ...
         if (ourVersion < remoteVersion) {
             ...
             if (QMessageBox::warning(nullptr, QObject::tr("FTSCoin Wallet Update"), QObject::tr("There is an update to the wallet available.\nWould you like to go to the download page?"), QMessageBox::Ok, QMessageBox::Cancel) == QMessageBox::Ok) {
                 QString link = "https://github.com/FTSCoinNetwork/FTSCoin-desktop/releases";
